...
__task void Soil_Task(void) {
    ...
    while (1) {
        adc = ADC_Read();
        m = ((ADC_DRY - adc) * 100) / (ADC_DRY - ADC_WET);
        if (m < 0) m = 0;
        if (m > 100) m = 100;
...
static void Dht_Init(void)  { DHT_PORT->FIODIR &= ~DHT_MASK; }
...
static void step_cw(void) {
    ...
    LPC_GPIO0->FIOSET = IN1|IN2;
    step_delay_us(2000);

    LPC_GPIO0->FIOCLR = IN1|IN2|IN3|IN4;
    LPC_GPIO0->FIOSET = IN2|IN3;
    step_delay_us(2000);

    LPC_GPIO0->FIOCLR = IN1|IN2|IN3|IN4;
    LPC_GPIO0->FIOSET = IN3|IN4;
    step_delay_us(2000);

    LPC_GPIO0->FIOCLR = IN1|IN2|IN3|IN4;
    LPC_GPIO0->FIOSET = IN4|IN1;
    step_delay_us(2000);
}
...
/* =========================================================
   -------------------- DC MOTOR ----------------------------
   ========================================================= */
#define DC_MOTOR_PIN (1 << 0)   // P1.20
#define MOISTURE_MIN 5           // % threshold
...
__task void DCMotor_Task(void) {
    ...
    while (1) {
        ...
        if (m > MOISTURE_MIN){
			LPC_GPIO2->FIOCLR = DC_MOTOR_PIN;   // motor ON
			sprintf(buf,"Fan : ON\r\n");
		}
		else{
			LPC_GPIO2->FIOSET = DC_MOTOR_PIN;   // motor OFF
			sprintf(buf,"Fan : OFF\r\n" );
		}
